...
// ── GET /api/retards — retards de l'école (?eleveId= pour filtrer) ────────────
// Un élève ne voit que ses propres retards.
export async function GET(request: NextRequest) {
  try {
    ...
    let query = supabase.from('retards').select('*, eleves!inner(ecole_id)').eq('eleves.ecole_id', ecoleId)
...
// ── POST /api/retards — enregistrer un retard (surveillant/admin) ─────────────
export async function POST(request: NextRequest) {
  try {
    ...
    const { eleveId, date, heureArrivee, motif, justifie } = await request.json()
    if (!eleveId || !date || !heureArrivee) {
      return NextResponse.json({ error: 'Élève, date et heure requis' }, { status: 400 })
    }
    ...
    const { data, error } = await supabase
      .from('retards')
      .insert({ eleve_id: eleveId, surveillant_id: session.sub, date, heure_arrivee: heureArrivee, motif: motif || null, justifie: !!justifie })
      .select('*')
      .single()
